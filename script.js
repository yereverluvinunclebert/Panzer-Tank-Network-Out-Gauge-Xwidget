/*
	Tank network OUT gauge Widget

	Created by Dean Beedell

	Face inspired by KJC66's Widget
...
function setMaxBandwidth() {       
    ...
    if (debugFlg == 1) {print("NetMonitorCore1.MaxNetIn"+ NetMonitorCore1.MaxNetIn);};
}
//=====================
//End function
//=====================
...
function getPrefs(){
    ...
    if (debugFlg == 1) {print("preferencesGaugeSizevalue.percent "+preferencesGaugeSizevalue.percent);};
    ...
    if (debugFlg == 1) {print("preferencesHintPrefValue.text "+preferencesHintPrefValue.text);};
...
function savePrefs()
{
         if (debugFlg == 1) {print ("Saving Prefs ");};
...
function menuitem12OnClick(Sender)
{
          ...
          {
              openURL("http://lightquick.co.uk/downloads/panzer-network-out-gauge-xwidget.html?Itemid=558");
          }
}
//=====================
//End function
...
//==============================================================
function settooltip() {
    if (debugFlg == 1) {print("settooltip");};
    if (debugFlg == 1) {print("preferencesHintPrefValue.text " + preferencesHintPrefValue.text);};
...
function checkLockWidget ()        {
           ...
           if (preferenceswidgetLockLandscapeModePrefvalue.text == "disabled") {
	        ...
	        //screw2.tooltip="click me to lock the widget in place";
                 ...
                 if (debugFlg == 1) {print("preferenceswidgetLockLandscapeModePrefvalue.text " +preferenceswidgetLockLandscapeModePrefvalue.text);};
                 pin.opacity = 255;
...
function checkNetUsage() {
   if (debugFlg == 1) {print("outNetText.text "+outNetText.text);};
    
   var dd = outNetText.text;
...
{
 var totalAmount = parseInt(fieldNetOutTotalK.text);
...
function switchOffFunctions()
{
    ...
    outPointer.visible = false;
    outPointerShadow.visible = false;
}


function tickSwitchOnMouseDown(Sender,Button,Shift,X,Y)
{
  //print("outPointer.enabledAnimateChange "+outPointer.enabledAnimateChange);
  if (preferencessoundprefvalue.text != "disabled" ) {PlaySound(lock);};
  if (preferencesActionSwitchPrefValue.text === "continuous" ) {
    preferencesActionSwitchPrefValue.text = "tick";
    outPointer.enabledAnimateChange=0;
    outPointerShadow.enabledAnimateChange=0;
  } else {
    preferencesActionSwitchPrefValue.text = "continuous" ;
    outPointer.enabledAnimateChange=-1;
    outPointerShadow.enabledAnimateChange=-1;
  }
  savePrefs();
}  
